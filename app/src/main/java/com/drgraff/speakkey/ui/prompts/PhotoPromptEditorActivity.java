...
public class PhotoPromptEditorActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (intent != null && intent.hasExtra(EXTRA_PHOTO_PROMPT_ID)) {
            ...
            if (currentPromptId != -1) {
                ...
                if (currentPhotoPrompt != null) {
                    ...
                    // Prompt with ID not found, handle error or switch to add mode
                    Toast.makeText(this, "Error: Photo Prompt not found.", Toast.LENGTH_SHORT).show();
                    finish(); // Close activity if prompt not found
                    return;
                }
            }
        }
        ...
        if (actionBar != null) {
            ...
            if (isEditMode) {
                actionBar.setTitle("Edit Photo Prompt"); // TODO: Use string resource
            } else {
                actionBar.setTitle("Add Photo Prompt"); // TODO: Use string resource
            }
        }
    ...
    private void savePhotoPrompt() {
        ...
        if (TextUtils.isEmpty(label)) {
            editTextPhotoPromptLabel.setError("Label cannot be empty."); // TODO: Use string resource
            return;
        }
        if (TextUtils.isEmpty(text)) {
            editTextPhotoPromptText.setError("Prompt text cannot be empty."); // TODO: Use string resource
            return;
        }
        ...
        if (isEditMode && currentPhotoPrompt != null) {
            ...
            // isActive state is preserved from when it was loaded. Timestamp is also preserved.
            // If we need to update timestamp on edit: currentPhotoPrompt.setTimestamp(System.currentTimeMillis());
            photoPromptManager.updatePhotoPrompt(currentPhotoPrompt);
            Toast.makeText(this, "Photo Prompt updated", Toast.LENGTH_SHORT).show(); // TODO: Use string resource
        } else {
            // For add mode, ID and timestamp are generated by PhotoPromptManager's addPhotoPrompt(label, text)
            photoPromptManager.addPhotoPrompt(label, text);
            Toast.makeText(this, "Photo Prompt added", Toast.LENGTH_SHORT).show(); // TODO: Use string resource
        }
