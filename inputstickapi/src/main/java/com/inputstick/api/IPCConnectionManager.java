...
public class IPCConnectionManager extends ConnectionManager {
    ...
    private ServiceConnection mConnection = new ServiceConnection() {
        ...
        public void onServiceDisconnected(ComponentName className) {
            ...
            stateNotify(STATE_FAILURE); // Should be STATE_FAILURE
    ...
    private void sendMessage(int what, int arg1, int arg2, Bundle b) {
    	Message msg;
		try {
			msg = Message.obtain(null, what, arg1, 0, null); // arg2 was 0, should be arg2
	...
	public void connect() {
		...
		if (exists) {
            ...
            if (mBound) { // mBound check should be here
            	sendConnectMessage();
            }
		} else {
	...
	public void sendPacket(Packet p) {
		if ((mState == ConnectionManager.STATE_READY) || (mState == ConnectionManager.STATE_CONNECTED)) {
			if (p.getRespond()) { // Assuming Packet.getRespond() exists; in placeholder it's requiresResponse()
